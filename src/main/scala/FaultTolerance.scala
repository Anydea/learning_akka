...
class Listener extends Actor with ActorLogging {
  import Worker._
  context.setReceiveTimeout( DurationInt(15).seconds )
  def receive = {
...
class Worker extends Actor with ActorLogging {
  ...
  implicit val askTimeout = Timeout(DurationInt(5).seconds)
  ...
  def receive = LoggingReceive {
    case Start if progressListener.isEmpty =>
      ...
      context.system.scheduler.schedule(Duration.Zero, DurationInt(1).second, self, Do)
...
class CounterService extends Actor {
  ...
  override val supervisorStrategy  = OneForOneStrategy(maxNrOfRetries =3, 
      withinTimeRange = DurationInt(5).seconds ){
  ...
  def receive = LoggingReceive {
      ...
      case Terminated(actorRef) if Some(actorRef) == storage =>
        ...
        context.system.scheduler.scheduleOnce(DurationInt(10).seconds, self, Reconnect)
...
object DummyDB {
  ...
  def save(key: String, value: Long): Unit = synchronized{
    if ( 11 <= value && value <= 14)
       throw new StorageException("Simulate store failure " + value)
    db += (key -> value)
  }
